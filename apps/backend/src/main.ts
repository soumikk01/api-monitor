...
function ok(label: string, detail = '')  { console.log(`${c.green}  ✔${c.reset}  ${c.bold}${label}${c.reset}${detail ? `  ${c.gray}${detail}${c.reset}` : ''}`); }
function warn(label: string, detail = '') { console.log(`${c.yellow}  ⚠${c.reset}  ${c.bold}${label}${c.reset}${detail ? `  ${c.gray}${detail}${c.reset}` : ''}`); }
function fail(label: string, detail = '') { console.log(`${c.red}  ✘${c.reset}  ${c.bold}${label}${c.reset}${detail ? `  ${c.gray}${detail}${c.reset}` : ''}`); }
...
async function bootstrap() {
  ...
  console.log('');
  console.log(`${c.cyan}${c.bold}  ⚡ API Nest — Backend Starting${c.reset}  ${c.gray}(${env})${c.reset}`);
  ...
  // ── 2. Redis check ────────────────────────────────────────────────────
  if (!redisUrl) {
    warn('Redis', 'REDIS_URL not set — caching and BullMQ queue disabled, WebSocket pub/sub disabled');
  } else {
  ...
  try {
    ...
    if (redisUrl) {
      ...
      warn('WebSocket', 'Socket.io running in single-node mode (no Redis adapter)');
    }
  } catch (err: any) {
    warn('WebSocket', `Redis adapter failed — falling back to in-memory: ${err?.message}`);
  }

  // ── 4. BullMQ Queue ───────────────────────────────────────────────────
  if (!redisUrl) {
    warn('BullMQ', 'Queue disabled — no Redis connection. Ingest jobs will fail.');
  } else {
  ...
  if (!process.env.JWT_SECRET || !process.env.JWT_REFRESH_SECRET) {
    fail('JWT', 'JWT_SECRET or JWT_REFRESH_SECRET not set — authentication will fail');
  } else {
  ...
  // ── Final ready banner ────────────────────────────────────────────────
  console.log('');
  console.log(`${c.green}${c.bold}  🚀 Backend ready on http://localhost:${port}${c.reset}`);
...
bootstrap().catch((err) => {
  console.error(`\n${c.red}${c.bold}  ✘ Fatal startup error:${c.reset}`, err?.message ?? err);
  console.error(`${c.gray}  Check your .env file — DATABASE_URL, REDIS_URL, JWT_SECRET${c.reset}\n`);
  process.exit(1);
});
